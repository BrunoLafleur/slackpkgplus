    ...
    if [ $(basename $1) = "GPG-KEY" ];then
      ...
      if $GPG --quiet $2|grep -q "$SLACKKEY" || [ "$STRICTGPG" == "off" ];then
        for PREPO in $(echo ${PRIORITY[*]}|sed 's/SLACKPKGPLUS_[^ ]*//g');do
          $GPG --quiet --output "${WORKDIR}/gpg/GPG-KEY-${PREPO}.gpg" --dearmor $2
        done
      else
        echo
        echo "                   !!! F A T A L !!!"
      ...
      for PREPO in ${REPOPLUS[*]};do
        ...
        if [ $? -eq 0 ];then
          ...
          $GPG --quiet --import $2-tmp-$PREPO
          $GPG --quiet --output "${WORKDIR}/gpg/GPG-KEY-${PREPO}.gpg" --dearmor $2-tmp-$PREPO
        else
          echo
          echo "                   !!! W A R N I N G !!!"
  ...
  function checkgpg() {
    ...
    if [ -e "${1}.asc" ];then
      ...
      if [ "$STRICTGPG" != "off" ] && ! echo ${MIRRORPLUS[$REPO]}|grep -q ^dir:/;then
        ...
        $GPG --quiet --verify ${1}.asc ${1} 2>/dev/null && echo "1" || echo "0"
      fi
